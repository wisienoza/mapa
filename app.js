// === APPLICATION LOGIC (wydzielone z index.html) ===
        ...
        function adjustLayout() {
            ...
            // +14 = 7 px (top .left-wrapper/.right-wrapper) + 7 px luzu nad paskiem flag.
            var _lootEl = document.getElementById('loot-wrapper');
            var _lootH = _lootEl ? _lootEl.offsetHeight : 56;
            const colH = ((window.innerHeight - (_lootH + 14)) / scale) + 'px';
        ...
        window.updateFactbookPanel = function(id, name) {
            ...
            try {
                    ...
                    fPanel.innerHTML = `
                        ${anthemUrl
                            ? `<a href="${anthemUrl}" target="_blank" rel="noopener" title="Posłuchaj hymnu narodowego (nationalanthems.info)" style="position:relative; display:block; text-decoration:none; color:inherit;"><img src="${flagUrl}" class="fact-img" alt="Flag" style="cursor:pointer;"><span class="ext-ico" style="position:absolute; top:6px; right:8px; margin-left:0; opacity:1; background:rgba(0,0,0,0.6); padding:1px 6px; border-radius:4px; font-size:0.85em;">🎵 ↗</span></a>`
